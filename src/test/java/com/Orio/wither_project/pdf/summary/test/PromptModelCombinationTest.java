...
import com.Orio.wither_project.process.summary.config.SummaryConstantsConfig;
import com.Orio.wither_project.process.summary.config.SummaryPromptConfig;
import com.Orio.wither_project.process.summary.service.generation.impl.OllamaSummaryGenerationService;
...
class PromptModelCombinationTest {
        ...
        @Autowired
        private SummaryConstantsConfig constantsConfig;
        ...
        void testAllCombinations() throws IOException {
                logger.info("Starting prompt-model combination test for models: {}",
                                models.getAllModels());
                ...
                try (CSVWriter writer = new CSVWriter(new FileWriter(resultPath.toFile()))) {
                        writer.writeNext(new String[] { "Model", "Prompt Type", "Generation #",
                                        "Summary",
                                        "Execution Time (ms)" });
                        ...
                        for (String model : models.getAllModels()) {
                                ...
                                OllamaSummaryGenerationService service = new OllamaSummaryGenerationService(chatModel,
                                                promptConfig, constantsConfig, objectMapper);

                                // Test different prompt types
                                testPromptType(service, "Executive",
                                                promptConfig.getExecutiveSummarySystemMessage(),
                                ...
                                testPromptType(service, "Technical",
                                                ...
                                                writer,
                                                model);
                                testPromptType(service, "Creative",
                                                promptConfig.getCreativeSummarySystemMessage(),
                                                promptConfig.getCreativeSummaryJsonSchema(), writer, model);
                                testPromptType(service, "Analytical",
                                                promptConfig.getAnalyticalSummarySystemMessage(),
                                                promptConfig.getAnalyticalSummaryJsonSchema(), writer, model);
                                testPromptType(service, "Narrative",
                                                promptConfig.getNarrativeSummarySystemMessage(),
        ...
        private void testPromptType(OllamaSummaryGenerationService service, String promptType,
                ...
                for (int i = 1; i <= GENERATIONS_PER_COMBINATION; i++) {
                        ...
                        String summary = service.summarize(testText, systemMessage.getContent());
                        long executionTime = System.currentTimeMillis() - startTime;
                        logger.debug("Generation completed in {}ms", executionTime);

                        TestResult result = new TestResult(model, promptType, i, summary,
                                        executionTime);
                ...
                logger.debug("Completed testing prompt type '{}' with model '{}'",
                                promptType, model);
        }
}
